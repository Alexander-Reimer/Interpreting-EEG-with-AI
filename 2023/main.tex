...
% Ergebnisse (Trainingsverlauf, Genauigkeiten, etc.)

% Vergleich Genauigkeit mit & ohne Dropout

% Vorgehensweise Architektur beschreiben
...
\begin{figure}[h!]
    ...
    \begin{axis}[
        ...
        ymax=2, no markers, legend pos=south east, every plot/.style = {line width = 2pt}]
...
%diese Differenz $\textrm{Ausgabe}_\textrm{ist} - \textrm{Ausgabe}_\textrm{soll}$ zu minimieren.
...
Bei einer Dropout-Wahrscheinlichkeit von 30\% werden also im Schnitt 30\% der Neuronen auf 0 gesetzt und somit ignoriert.  % Quelle?
...
\begin{figure}
    ...
    \begin{tikzpicture}[
        ...
        \node[rotate=90, anchor=north, align=center] (conv1) {\layerDesc{1D Conv}{$\textrm{Dim}_K$ = 3, Kanäle = 64}};
        \node[layer=conv1] (drop1) {\layerDesc{Dropout}{$P = 20\%$}};
        \node[layer=drop1] (maxpool1){\layerDesc{Max Pool}{$n = 2$}};
        \node[layer=maxpool1] (conv2) {\layerDesc{1D Conv}{$\textrm{Dim}_K$ = 2, Kanäle = 32}};
        \node[layer=conv2] (maxpool2) {\layerDesc{Max Pool}{$n = 2$}};
        \node[layer=maxpool2] (conv3) {\layerDesc{1D Conv}{Filtergröße = 2, Kanäle = 16}};
        ...
        \node[layer=drop2] (dense1) {\layerDesc{Fully Connected}{Neuronen = 16}};
        \node[layer=dense1] (dense2) {\layerDesc{Fully Connected}{Neuronen = 16}};
        \node[layer=dense2] (dense3) {\layerDesc{Fully Connected}{Neuronen = 64}};
        \node[layer=dense3] (dense4) {\layerDesc{Fully Connected}{Neuronen = 16}};
        \node[layer=dense4] (dense5) {\layerDesc{Fully Connected}{Neuronen = 3}};
        \draw[-stealth]
            (conv1) \arr (drop1)
            (drop1) \arr (maxpool1)
            (maxpool1) \arr (conv2)
            ...
            ;
    \end{tikzpicture}
    \caption{Struktur von unserem aktuell besten neuronalen Netzwerk. $\textrm{Dim}_K$ steht für die Größe des Kernels, 1d Conv für 1-dimensionale Convolutional Schicht.}
