...
function blink_fft()
    path = "model_data/visual_cortex_eyes/Blink/Okzipital-03-13-2022/"
    ...
    for i = 2:2
        # current_d = reshape(mapslices(average, data_x, dims=[2]), (400,))
        current_d = data_x[:, i]
        current_d = divide_into_channels(current_d, 2)

        ax1 = plt.subplot(121, title = "Originale EEG Daten")
        xlabel("Zeit in 5ms")
        ylabel("Spannungsdifferenz in Mikrovolt")
        ax1.plot(current_d[1], color = "red")
        ax1.plot(current_d[2], color="blue")

        ax2 = plt.subplot(122, title = "FFT (Spektralanalyse)")
        xlabel("Frequenz in Hertz")
        ylabel("Amplitude")
        ...
        @info size(y_fft_2[2])
        ax2.plot(abs.(y_fft_1[2]))
        ax2.plot(abs.(y_fft_2[2]))
        ...
        # y_ifft_1 = irfft(y_fft_1[2], 200)
        # y_ifft_2 = irfft(y_fft_2[2], 200)
        # #ax2.plot(x_fft_1, y_fft_1, color = "red")
        # ax3.plot(y_ifft_2)
    end
end
